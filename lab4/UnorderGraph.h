#pragma once
#ifndef UnorderGraph_h
#define UnorderGraph_h
...
template <class T>
class UnorderGraph : public Graph<T>{
private:
...
public:
    UnorderGraph() : matrix(new std::vector<std::vector<T> *>), names(new std::vector<std::string>){};
    ...
    void DFS(std::vector<int>* used, std::vector<std::vector<int>>* edges, int v) {
        used->operator[](v) = 1;
        for (auto to : edges->operator[](v)) {
            if (!used->operator[](to)) {
                DFS(used, edges, to);
            }
        }
    ...
    int CountComponents(){
        std::vector<std::vector<int>>* edge_matrix = new std::vector<std::vector<int>>(size);
        for (int i = 0; i < size; ++i){
            for (int j = 0; j < size; ++j){
                if (matrix->operator[](i)->operator[](j)){
                    edge_matrix->operator[](i).push_back(j);
                }
            }
        }
        std::vector<int>* used = new std::vector<int>(size, 0);
        int amount = 0;
        for (int i = 0; i < size; ++i){
            if (!used->operator[](i)) {
                amount++;
                DFS(used, edge_matrix, i);
            }
        }
        return amount;
    }
